 ...
 * @var string
 */
 protected $signature = 'pos:reading 
                        {--date=now : YYYY-MM-DD}
                        {--brcode=egc : branch code}
                        {--mode=x : branch code}
                        {--terminal=P01 : print z reading}';
...
public function handle()
{
    ...
    */
    ...
    $date = $this->option('date')=='now' ? Carbon::now() : Carbon::parse($this->option('date'));

    $this->info($this->option('terminal'));
    if (in_array(strtolower($this->option('terminal')), ['p01', 'p02', 'p03'] ))
      $terminal = strtoupper($this->option('terminal'));
    else {
      $this->info('invalid terminal'); 
      exit;
    }
    ...
    if (in_array(strtolower($this->option('mode')), ['x', 'z'] ))
      $mode = strtoupper($this->option('mode'));
    else {
      $this->info('invalid mode'); 
      exit;
    }
    ...
    $this->info($date->format('Y-m-d').' '.$mode.' '.$terminal);
    ...
    $datas = $this->getTzreading($date, $terminal, $page, $mode);
    $this->zreadfile($date, $datas, $terminal);
  ...
  private function getZreading(Carbon $date) {
    ...
    return $this->getLines2($date, $invhdr, $orpaydtls);
      
  }

  private function getTzreading(Carbon $date, $terminalid, $page=NULL, $mode) {
    ...
    return $this->getLines2($date, $invhdr, $orpaydtls, $terminalid, $page, $mode);
  ...
  private function getLines2($date, $invhdr, $orpaydtls, $terminalid=NULL, $page=NULL, $mode) {
    ...
    array_push($lines, bpad('* END OF '.$mode.'-REPORT *', 40));
    //array_push($lines, bpad(c()->format('m/d/Y H:i:s'), 40));
    if (!is_null($page))
      array_push($lines, bpad($page, 40));
  ...
  private function setToArray($is_new, $key, $invdtl, $combo=null, $table='invdtl', $cancelled=false) {
    ...
    return [
      ...
      '', //COMPQTY5
      '', //COMPUNIT1
      substr($menucat or '', 0, 6), //COMPUNIT2
      substr($menucat or '', 6, 6), //COMPUNIT3
