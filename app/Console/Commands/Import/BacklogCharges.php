...
class BacklogCharges extends Command
{
  ...
  protected $signature = 'import:backlog-charges {from : from date} {to : to date} {brcode : Branch Code} {--reset= : reset}';
  ...
  protected $type = 4;
  protected $notes = 'backlog:charges brcode date';
    ...
    foreach ($this->dr->monthInterval() as $key => $date) {
      ...
      if ($locator->exists($path)) {
        ...
        $attr = [
          ...
          'path'      => $path,
          'type'      => $this->type,
          'note'      => 'backlog:charges '.stl($br->code).' '.$eom->format('Y-m-d')
        ];
