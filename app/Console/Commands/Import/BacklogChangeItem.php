...
class BacklogChangeItem extends Command
{
  ...
  protected $signature = 'import:change-item {from : from date} {to : to date} {brcode : Branch Code} {--reset= : reset}';
  ...
  protected $type = 5;
  protected $notes = 'import:change-item brcode date';
    ...
    foreach ($this->dr->monthInterval() as $key => $date) {
      ...
      if ($locator->exists($path)) {
        ...
        $attr = [
          ...
          'path'      => $path,
          'type'      => $this->type,
          'note'      => 'import:change-item '.stl($br->code).' '.$eom->format('Y-m-d')
        ];
