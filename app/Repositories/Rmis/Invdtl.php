...
class Invdtl extends BaseRepository implements CacheableInterface
{
  ...
  public function whereDate(Carbon $date) {
      ...
      return $query->where('a.date', $date->format('Y-m-d'))
                    ...
                    ->select(DB::raw('i.*, (a.vtotal+a.xtotal+a.ztotal) as subtotal, a.refno, a.date, a.timestop, a.tableno, a.tablename, a.pax, a.saletype, a.scpax, a.scdisc, a.pwddisc, a.vatamount, a.svcamount, a.uidcreate, a.vtotal, a.discountid, a.discamount, b.refno as ordrefno, b.time as ordtime'))
                    ->groupBy('i.id')
                    ->orderBy('a.refno')
                    ->orderBy('i.lineno');
    });
