...
class UploaderController extends Controller 
{
  ...
  protected $doctype;
	...
	private function processBackup(Request $request) {
		...
		if ($backup_date) { // check if filename (GC040616.ZIP) is valid date 
          ...
          if ($backup_date->gt(Carbon::parse('2021-10-31'))) { // dont verify branch if backup is generated by 5-1
          ...
          */
          ...
          event(new \App\Events\Process\AggregatorMonthly('trans-component', $backup->date, $backup->branchid));  // update ms_component
  ...
  private function verifyBackup(Request $request) {
  	try {
  		$res = $this->posUploadRepo->getBackupCode(); 
  	} catch (\Exception $e) {
  		throw $e;
  	}
  	
  	if (strtolower($res['code'])===strtolower($request->user()->branch->code)) {
  		return $res['code'];
  	} else {
  		throw new Exception("Backup file is property of ". $res['code'] .' not '.$request->user()->branch->code);
  	}
  }
